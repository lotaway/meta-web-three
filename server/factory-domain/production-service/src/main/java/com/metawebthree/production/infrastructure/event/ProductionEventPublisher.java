...
public class ProductionEventPublisher {
    ...
    public void publishOrderCreated(ProductionOrder order) {
        String message = String.format("{\"event\":\"ORDER_CREATED\",\"orderCode\":\"%s\",\"productCode\":\"%s\"}",
            order.getOrderCode(), order.getProductCode());
        kafkaTemplate.send("production.events", message);
    ...
    public void publishOrderCancelled(ProductionOrder order) {
        String message = String.format("{\"event\":\"ORDER_CANCELLED\",\"orderCode\":\"%s\"}", order.getOrderCode());
        kafkaTemplate.send("production.events", message);
