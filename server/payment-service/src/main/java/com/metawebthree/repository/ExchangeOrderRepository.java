...
public interface ExchangeOrderRepository extends BaseMapper<ExchangeOrder> {
    ...
    List<ExchangeOrder> findByStatusAndCreatedAtBefore(@Param("status") String status, @Param("time") Timestamp time);
    ...
    BigDecimal getTotalCompletedAmountByUserIdAndDateRange(@Param("userId") Long userId, @Param("startDate") Timestamp startDate);
    
    @Select("SELECT COUNT(*) FROM Exchange_Orders WHERE user_id = #{userId} AND status = 'COMPLETED' AND created_at >= #{startDate}")
    Long getCompletedOrderCountByUserIdAndDateRange(@Param("userId") Long userId, @Param("startDate") Timestamp startDate);
    ...
    List<ExchangeOrder> findByCreatedAtBetween(@Param("startDate") Timestamp start, @Param("endDate") Timestamp end);
