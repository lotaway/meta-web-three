...
contract GoodsNFT is ERC721, Ownable {
    ...
    function mint(uint256 tokenId, address referrer) external {
        ...
        require(metaThreeCoin.transferFrom(msg.sender, address(this), price), "Payment failed");
        ...
        if (referrer != address(0)) {
            ...
            require(commissionRelation.isValidDistributor(referrer), "Invalid referrer");
            commissionRelation.setUpline(msg.sender, referrer);
            
            // Distribute commission to buyer
            require(commissionToken.mint(msg.sender, commission), "Commission mint failed");
            
            // Distribute commission to uplines
            address[] memory uplines = commissionRelation.getUplines(msg.sender);
            uint256 uplineCommission = (commission * UPLINE_COMMISSION_RATE) / RATE_DENOMINATOR;
            
            for(uint i = 0; i < uplines.length; i++) {
                if(uplines[i] == address(0)) break;
                require(commissionToken.mint(uplines[i], uplineCommission), "Upline commission mint failed");
            }
        }
    ...
    function getGoodSpecifications(uint256 tokenId) external view returns (Specification[] memory) {
        require(_exists(tokenId), "Good does not exist");
