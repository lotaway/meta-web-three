    ...
    pub async fn set_online(&mut self) {
        ...
        tokio::task::spawn(async move {
            while let Some(block_from_other) = rx.recv().await {
                let mut _self = Arc::clone(&_self).lock().await;
        ...
        let peers_clone = Arc::clone(&peers);
        tokio::task::spawn(async move {
            let block_chain_guard = self.block_chain.as_ref().expect("Can't found block chain to lock").lock().await;
...
type TFutureTaskResult<'a> = Result<&'a Arc<Mutex<BlockChain>>, LinkSummonProxyError>;

impl<'a> TFutureTask<TFutureTaskResult<'a>> for LinkSummonProxy {
    async fn start(&mut self) -> TFutureTaskResult<'a> {
