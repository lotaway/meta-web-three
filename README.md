...
4. **前置的依赖与公共资源**：如果新服务依赖 PostgreSQL/Redis/RabbitMQ 等基础组件，通常不需要新增 yaml；若你的服务需要额外 ConfigMap/Secret 或存储卷，再分别在 `k8s/configmaps/`、`k8s/secrets/`、`k8s/storage/` 中补齐。
...
See [DEVELOPMENT.md](./DEVELOPMENT.md) for the full development workflow, code principles, and end-to-end guidance.