...
module.exports = (config, { 
  ...
  if (wechatAppId) {
    config = withWechatConfig(config, wechatAppId)
  }

  if (alipayAppId) {
    config = withAlipayConfig(config, alipayAppId)
  }

  return config
}

function withWechatConfig(config, appId) {
...
function withAlipayConfig(config, appId) {
  config = withInfoPlist(config, c => {
    c.modResults.CFBundleURLTypes = c.modResults.CFBundleURLTypes || []
    c.modResults.CFBundleURLTypes.push({
      CFBundleURLSchemes: [`alipay${appId}`]
    })
    return c
  })
