...
- [ ] **代码审查修复：payment-service** — 使用自定义业务异常 ExternalServiceException 替代 RuntimeException，删除 TODO 注释，编译通过
  - ⚠️ 问题：CryptoWalletServiceImpl.java (10处) 和 PriceEngineServiceImpl.java (2处) 仍有多处 RuntimeException 未替换为 BusinessException
- [x] **代码审查修复：order-service（中文注释）** — 删除所有中文注释，编译通过
- [ ] **代码审查修复：order-service（语法错误）** — 删除多余闭合大括号，修复 Java 源文件语法错误，编译通过
  - ❌ 未通过：OrderPaymentSaga.java (5处) 和 SagaOrchestratorImpl.java 仍有 RuntimeException 未替换为 BusinessException
- [ ] **代码审查修复：user-service** — 运行 make gen-java-dubbo 重新生成 RPC 代码，编译通过
  - ❌ 未通过：UserServiceImpl.java 仍有 2 处 RuntimeException，第394行有中文错误信息"修改密码失败"
- [x] **代码审查修复：supplier-service** — 使用自定义业务异常 BusinessException 替代 RuntimeException，所有错误信息改为英文，编译通过
...
- [ ] **服务间调用统一使用 Dubbo RPC** — 所有服务间调用必须通过 `@DubboReference` 走 Dubbo Triple 协议，禁止使用 RestTemplate + 硬编码 URL
  ...
  - ✅ 已删除：order-service/RestTemplateConfig.java 配置类
...
- [ ] **服务目录检查：recommendation-service** — @server/mall-domain/recommendation-service/和 @server/ai-domain/recommendation-service/ 重复作用，不管技术栈是否不同，都必须合并
- [ ] **修正：Maven编译错误** — supply-chain-domain/supplier-service 和 mall-domain/supplier-service 重复定义导致编译失败
  - ✅ 已修复：将 mall-domain/supplier-service 的 artifactId 改为 mall-supplier-service
  - ⚠️ 待验证：需编译验证 Maven DuplicateProjectException 是否解决