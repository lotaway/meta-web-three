...
contract AboutUseLib {
    ...
    function useLib() public {
        insideMethod.some();    //  normal call
        num.sub(10); //  uint var direct call
    }
...
contract UnitAndGlobalData {
    ...
    function action(address payable addr) public payable {
        ...
        // address(this).call();
        selfdestruct(addr); //  destruct this contract and transfer money to param address
    }
}
...
contract ChildContract is AboutFunction(1), Func2, Func3 {
    ...
    function noReadState(string memory str) public returns(string memory) {
        ...
        super.willBeOverride(); //  call parent function
        willBeOverride();   //  call function (already override by child)
        return str;
    }
    ...
    function insetFunc(bytes calldata str1) public pure returns(uint result1, uint result2, bytes32 encodeStr1, bytes32 encodeStr2, bytes20 encodeStr3) {
        result1 = addmod(1, 2, 3);    //  (1 + 2) % 3
        result2 = mulmod(1, 2, 3);    //  (1 * 2) % 3
