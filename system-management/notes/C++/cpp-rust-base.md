...
Rust还有另一特点是没有Null值，而是使用枚举Option::Some/None来指代结果可能为空的情况，类似于C++的std::optional<T>可选值，Rust大量使用Option和基于此的Result来指定参数与返回值的可选性。
...
以上声明等同于`let num1: i32 = 1;`，`let mut num2: i32 = 1;`
...
C++作为传统老语言，依赖引入方式除了内置的标准库可以直接使用`#include <libraryName>`宏引入外，其他第三方或者自行开发的库都需要通过一套复杂流程才能引入使用，包括将所需库文件放入项目/解决方案内，添加链接器指向的依赖文件路径，最后才能在代码里同样使用`#include`宏引入。
